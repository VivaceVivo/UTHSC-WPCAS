...
if ( !class_exists('WPSSO') ) {
	...
	class WPSSO {
		...
		public function __construct() {
			...
			if ( get_option( 'wpsso_host' ) ) {
				error_log("initializing SSO: " . get_option( 'wpsso_host' ), 0);
				//Initialize phpCAS
		...
		protected function wp_cas_authentication_hooks(){
			//add_action('init', array('WPSSO', 'lock_down_check'));
			add_filter('authenticate', array(&$this, 'authenticate'), 10, 3);
			add_filter('wp_signon', array(&$this, 'authenticate'),10,3);

			add_action('wp_logout', array(&$this, 'logout'));
			add_action('lost_password', array('WPSSO', 'disable_function'));
			add_action('retrieve_password', array('WPSSO', 'disable_function'));
			add_action('password_reset', array('WPSSO', 'disable_function'));
			add_filter('show_password_fields', array(&$this, 'show_password_fields'));
			//add_action('check_passwords', array('WPSSO', 'check_passwords'), 10, 3);
		...
		public function add_options_pages() {
			...
			add_menu_page('WP SSO', 'WP SSO', 'administrator', 'wpsso-settings', 'wpsso_preferences', $icon, '98.9');
			add_submenu_page('wpsso-settings', 'CAS Test', 'CAS Test', 'administrator', 'wpsso-test', 'wpsso_test');
			add_submenu_page('wpsso-settings', 'About', 'About', 'administrator', 'wpsso_about', 'wpsso_about');		
		}
		...
		public function wpcas_login_url($redirect = '', $force_reauth = false) {
			...
			$pattern = get_option('wpsso_native_login_url_pattern');
			if($pattern){
			...
			return 'https://'. get_option('wpsso_host') .":" . get_option('wpsso_port')  . get_option('wpsso_context') . '/login?service='. $login_url;
		...
		public function wpcas_register_url($redirect = '', $force_reauth = false) {
			...
			return 'https://'. get_option('wpsso_host') .":" . get_option('wpsso_port')  . get_option('wpsso_context') . '/register?service='. $redirect_url;
		...
		protected function initialize_phpCAS() {
			...
			// Initialize phpCAS
			phpCAS::client(SAML_VERSION_1_1, get_option('wpsso_host'),intval(get_option('wpsso_port')), get_option('wpsso_context'));
		...
		public function authenticate($login_url) {
			...
			if ( phpCAS::isAuthenticated() ) {
				...
				'user_login'		=>	$cas_user,
				'last_name'			=>	$cas_attributes[get_option('wpsso_last_name')],
				'first_name'		=>	is_array( $cas_attributes[get_option('wpsso_first_name')] ) ? $cas_attributes[get_option('wpsso_first_name')]['1'] : $cas_attributes[get_option('wpsso_first_name')],
				'user_email'		=>	$cas_attributes[get_option('wpsso_user_email')],
			    'user_nicename'		=>	(get_option('wpsso_nickname')) ? 
			            (isset($cas_attributes[get_option('wpsso_nickname')]) ? $cas_attributes[get_option('wpsso_nickname')] : 
			            	"") : ""
				);
				...
				} else if(get_option('wpsso_update_acct') == 'on'){
				...
				return 'https://'. get_option('wpsso_host') . get_option('wpsso_context') . '/login?service='. $login_url;
		...
		public static function readCasUID() {
			$cas_user = phpCAS::getUser();
			$cas_attributes = phpCAS::getAttributes();
			$uid_option = get_option('wpsso_uid');
...
register_activation_hook( __FILE__, array( 'WPSSO', 'activate' ) );
register_deactivation_hook( __FILE__, array( 'WPSSO', 'deactivate' ) );
register_uninstall_hook( __FILE__, array( 'WPSSO', 'uninstall' ) );
